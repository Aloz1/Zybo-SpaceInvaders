...
proc init_gui { IPINST } {
  ...
  set_property tooltip {Settings related to AXI interface} ${Page_0}
  ...
  #Adding Page
  set Pin_settings [ipgui::add_page $IPINST -name "Pin settings"]
  set_property tooltip {Settings related to input and output pins} ${Pin_settings}
  set C_IRQ_ACTIVE_STATE [ipgui::add_param $IPINST -name "C_IRQ_ACTIVE_STATE" -parent ${Pin_settings}]
  ...
  set C_IRQ_SENSITIVITY [ipgui::add_param $IPINST -name "C_IRQ_SENSITIVITY" -parent ${Pin_settings}]
  set_property tooltip {Sensitivity of IRQ: 0 - EDGE, 1 - LEVEL} ${C_IRQ_SENSITIVITY}
  ipgui::add_param $IPINST -name "C_CLOCK_PERIOD_TICKS" -parent ${Pin_settings}
  ipgui::add_param $IPINST -name "C_ACK_WAIT_TICKS" -parent ${Pin_settings}


}

proc update_PARAM_VALUE.C_ACK_WAIT_TICKS { PARAM_VALUE.C_ACK_WAIT_TICKS } {
	# Procedure called to update C_ACK_WAIT_TICKS when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.C_ACK_WAIT_TICKS { PARAM_VALUE.C_ACK_WAIT_TICKS } {
	# Procedure called to validate C_ACK_WAIT_TICKS
	return true
}

proc update_PARAM_VALUE.C_CLOCK_PERIOD_TICKS { PARAM_VALUE.C_CLOCK_PERIOD_TICKS } {
	# Procedure called to update C_CLOCK_PERIOD_TICKS when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.C_CLOCK_PERIOD_TICKS { PARAM_VALUE.C_CLOCK_PERIOD_TICKS } {
	# Procedure called to validate C_CLOCK_PERIOD_TICKS
	return true
}
...
proc update_MODELPARAM_VALUE.C_CLOCK_PERIOD_TICKS { MODELPARAM_VALUE.C_CLOCK_PERIOD_TICKS PARAM_VALUE.C_CLOCK_PERIOD_TICKS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.C_CLOCK_PERIOD_TICKS}] ${MODELPARAM_VALUE.C_CLOCK_PERIOD_TICKS}
}

proc update_MODELPARAM_VALUE.C_ACK_WAIT_TICKS { MODELPARAM_VALUE.C_ACK_WAIT_TICKS PARAM_VALUE.C_ACK_WAIT_TICKS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.C_ACK_WAIT_TICKS}] ${MODELPARAM_VALUE.C_ACK_WAIT_TICKS}
}
